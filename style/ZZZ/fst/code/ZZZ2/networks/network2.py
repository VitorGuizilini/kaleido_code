...
class NetworkBase:
    ...
    type , cnt , pars_calc , pars_eval = '' , 0 , [] , []
    ...
    def instance_norm( self , net , name = None ):
        ...
        if self.type == 'calc':
            ...
            self.pars_calc.append( mu )
            self.pars_calc.append( sigma_sq )
        elif self.type == 'eval':
            mu       = tf.placeholder( tf.float32 , [ None , None , None , None ] )
            sigma_sq = tf.placeholder( tf.float32 , [ None , None , None , None ] )
            self.pars_eval.append( mu )
            self.pars_eval.append( sigma_sq )

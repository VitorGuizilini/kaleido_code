...
#            for iter in range( args.num_iters ):
            for iter in range( int( args.num_iters / 2 ) ):
                ...
                for j , path in enumerate( self.ycL[ curr:last ] ):
                    ...
                    h , w , c = yc[j].shape ; n = int( np.ceil( max( h , w ) / size ) )
                    yc_small.append( scipy.misc.imresize( yc[j] , 1.0 / n , interp = 'nearest' ) )
                ys = self.ysL[ iter % len( self.ysL ) ]

                self.sess.run( [ optim ] ,
                        feed_dict = { self.yc1 : yc_small , self.yc2 : yc ,
                ...
                if self.time_to_eval( iter ):
                    L_full , L_style , L_content , L_totvar = self.sess.run(
                        ...
                        feed_dict = { self.yc1 : yc_small , self.yc2 : yc , self.ys  : ys } )

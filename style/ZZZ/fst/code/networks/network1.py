...
### CONV LAYER
def conv_layer( net , num_filters , filter_size , strides ,
                padding = 'SAME' , relu = True , name = None ):

    weights_init = conv_init_vars( net , num_filters , filter_size , name = name )
    strides_shape = [ 1 , strides , strides , 1 ]

    net = tf.nn.conv2d( net , weights_init , strides_shape , padding = padding )
    net = instance_norm( net , name = name )
...
def conv_tranpose_layer( net , num_filters , filter_size , strides ,
    ...
    weights_init = conv_init_vars( net , num_filters , filter_size , name = name , transpose = True )
    strides_shape = [ 1 , strides , strides , 1 ]
...
### RESIDUAL BLOCK
def residual_block( net , filter_size = 3 , name = None ):

    batch , rows , cols , channels = kld.get_shape( net )
    tmp =  conv_layer( net , 128 , filter_size , 1 , padding = 'VALID' , relu = True  , name = name + '_1' )
    return conv_layer( tmp , 128 , filter_size , 1 , padding = 'VALID' , relu = False , name = name + '_2' ) \
                    + tf.slice( net , [ 0 , 2 , 2 , 0 ] , [ batch , rows - 4 , cols - 4 , channels ] )

### CONV INIT VARS
def conv_init_vars( net , out_channels , filter_size ,
...
### INSTANCE NORM
def instance_norm( net , name = None ):
...
### REFLECT PADDING
def reflect_padding( net , pad = 40 ):

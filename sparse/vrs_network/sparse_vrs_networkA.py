...
class vrs( kld.tf.vrs ):
    ...
    def build( self , data ):
        ...
        with tf.variable_scope( 'Placeholders' ):
            ...
            self.input = kld.tf.plch( data.shape( 1 , with_none = True ) , 'input' )
            self.label = kld.tf.plch( data.shape( 2 , with_none = True ) , 'label' )
        ...
        with tf.variable_scope( 'Layers' ):
            ...
            deconv2 = deconv( deconv3 , 'deconv2' ,  32 ,   [5,3] , conv2 , both , both )
            deconv1 = deconv( deconv2 , 'deconv1' ,  32 , [7,5,3] , conv1 , both , both )
            output  = deconv( deconv1 ,  'output' ,   1 , [9,7,5] , input , both , sigm )

            self.output = tf.squeeze( output , axis = 3 )
